...
<section class="flex h-lvh" x-data="{ open: false}">
    ...
    <nav class="bg-white py-8 px-5 max-lgm:absolute w-80 border border-gray-300 shadow-sm h-full hidden md:block navbar">
        ...
        <ul>
            ...
            <li class="font-semibold mt-4 cursor-pointer">
                <x-admin.admin-navlink class='flex items-center justify-between py-2 px-3 rounded-sm' :url="request()->is('admin/packages') || request()->is('admin/packages/create') || request()->is('admin/packages/create/*')">
    ...
    <main class="px-5 w-full overflow-y-scroll">
        <section class="mx-auto lgm:max-w-4xl">
            ...
            <div class="relative">
                ...
                <div class="bg-white border border-gray-300 absolute -left-10 top-8" x-show="open" x-on:click.outside="open = false" x-cloak>
                    <form action="/logout" method="POST">
                        @csrf
                        <button class="navlink-btn text-red-500 fill-red-500"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M16 13v-2H7V8l-5 4 5 4v-3z"></path><path d="M20 3h-9c-1.103 0-2 .897-2 2v4h2V5h9v14h-9v-4H9v4c0 1.103.897 2 2 2h9c1.103 0 2-.897 2-2V5c0-1.103-.897-2-2-2z"></path></svg> Logout</button>
                    </form>
                    </div>
                </div>
            </div>
