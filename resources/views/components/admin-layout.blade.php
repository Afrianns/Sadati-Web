<x-header></x-header>
<section class="flex h-lvh" x-data="{ open: false, sidebar: false }" x-cloak>
    ...
    <nav class="bg-white py-8 px-5 max-lgm:absolute w-80 border border-gray-300 shadow-sm" :class=" sidebar ? 'h-full' : 'max-lgm:-translate-x-full'">
        ...
        <ul>
            <li class="font-semibold mt-4 cursor-pointer">
                ...
                <div class="mt-2 text-gray-600 font-light" x-show="open">
                    <p class="hover:bg-gray-100 py-2 px-3 rounded-sm">
                        <x-navlink class="text-sm px-9" href='/admin/confirm' :url="request()->is('admin/confirm')">Butuh Konfirmasi</x-navlink>
                    </p>
                    <p class="hover:bg-gray-100 py-2 px-3 rounded-sm">
                        <x-navlink class="text-sm px-9" href='/admin/confirmed' :url="request()->is('admin/confirmed')">Terkonfirmasi</x-navlink>
                    </p>
                </div>
            </li>
    ...
    <main class="mx-auto lgm:max-w-4xl px-5 w-full overflow-y-scroll">
        <div class="my-9 flex justify-between items-center"x-data="{open: false}">
            ...
            <div class="relative">
                ...
                <div class="bg-white px-2 border border-gray-300 absolute -left-6 top-8" x-show="open" x-on:click.outside="open = false">
